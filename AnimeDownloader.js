...
(function() {
	...
	function animePaheClicker() {
		let clickHd = setInterval(() => {
            let downloadButton = document.getElementById("pickDownload")
            let hdIndex = downloadButton.childElementCount - 1
			if (downloadButton.children[hdIndex].href != null
                && downloadButton.children[hdIndex].href != '') {
				window.open(downloadButton.children[hdIndex].href)
				clearInterval(clickHd)
			}
		}, 100)
	...
	function paheClicker() {
		let checkIfContinueExist = setInterval(() => {
            let continueButton = document.getElementsByClassName('col-sm-6')[0]
			if (continueButton.innerText == "Continue") {
				continueButton.children[0].click()
